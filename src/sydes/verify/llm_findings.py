...
_CODE_FINDINGS_HEADER = (
    ...
    "- an authorization/security control that the supplied code shows was removed or bypassed "
    "(never merely a change to security-sensitive code — see the security rule below)\n"
    ...
    "\n"
    "EVIDENTIARY STANDARD — before you emit a finding, you must be able to state all four, each "
    "grounded in the supplied evidence, not assumed:\n"
    "1. The exact changed operation or logic that is incorrect.\n"
    ...
    "security-sensitive (credentials, permissions, tokens, access control, patch/update "
    "endpoints). Before you write the word 'unauthorized', 'unintended', or 'without "
    "authorization' anywhere in a finding, stop and check: have you actually shown, from the "
    "supplied code, a check that runs and fails to stop the bad case? If not, you are inferring, "
    "not observing — do not emit the finding. A security finding requires the supplied code to "
    "directly show one of:\n"
    "- a required check was removed\n"
    "- a check is present but bypassed (e.g. an early return, a wrong condition, a "
    "short-circuit)\n"
    "- untrusted input visibly reaches a dangerous operation with no guard in the supplied code\n"
    "- access-control logic is inverted\n"
    "- a previously enforced restriction is removed, AND the supplied code additionally shows a "
    "caller, guard, or check elsewhere that depended on that restriction for its own correctness. "
    "The restriction sitting in security-sensitive code, or matching a name like 'credentials' or "
    "'password', is NOT such evidence by itself — a narrowed deny-list or path pattern is a "
    "data/config change, not proof of a missing authorization mechanism, unless the supplied code "
    "shows something else that relies on it.\n"
    "Intentionally relaxing a restriction (a deny-list narrowed, a validation loosened, a check "
    "moved) is a deliberate behavior change, not a defect on its own. Treat a test in the diff "
    "whose expectation flipped from error/failure to success for this exact behavior as direct "
    "evidence the change is intended and verified — that is a reason to stay silent, never a "
    "reason to report, no matter how the resulting behavior sounds. If the diff and its own tests "
    "show the new behavior was the intended change, and nothing in the supplied code shows a "
    "control was removed elsewhere or bypassed, do not report it — 'this could now allow X "
    "without authorization' is exactly the unsupported claim this rule exists to block, even at "
    "low confidence.\n"
    "Worked example — a request handler narrows a deny-list from a broad path pattern to a "
    "narrower one before applying a patch operation, and the diff's own test changes from "
    "asserting the operation fails to asserting it succeeds for a path outside the new narrower "
    "pattern. The supplied code shows no other caller or guard that depended on the old, broader "
    "pattern. Correct answer: no finding — the restriction change is deliberate and the test "
    "proves it, and nothing in the supplied code shows an authorization mechanism was bypassed.\n"
    "\n"
    "VALID findings (the supplied evidence alone proves the defect):\n"
    "- changed code dereferences a nullable value before an existing nil check runs\n"
    "- a loop condition can never become false for a state the supplied code demonstrates\n"
    "- a denominator can be zero from an input path visible in the supplied code\n"
    "- a changed branch reverses an invariant visible in the supplied code\n"
    "- a lock is released before a shared-state mutation the supplied code also shows executing "
    "concurrently\n"
    "- an authorization check is visibly deleted or bypassed in the supplied diff\n"
    "INVALID (reject these even if they sound plausible) — none of these may be reported, because "
    "each depends on something the supplied evidence does not establish:\n"
    "- \"this may be unsafe\" / \"this could be risky\"\n"
    "- \"authorization may be insufficient elsewhere\"\n"
    "- \"this could introduce a race\" (with no concurrent execution shown in the supplied code)\n"
    "- \"this could break transactions\"\n"
    "- \"this may allow unauthorized access\"\n"
    "- \"this security-sensitive operation should probably have validation\"\n"
    "- \"this invariant might not hold in other callers\"\n"
    "\n"
    "Rules:\n"
    ...
    "makes it fail.\n"
    "- `explanation` must name the exact changed operation and why it is incorrect. `impact` must "
    ...
    "Returning an empty list is the correct, expected answer, not a failure.\n"
