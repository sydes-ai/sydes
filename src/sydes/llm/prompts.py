...
def build_endpoint_discovery_prompt(
    ...
    *,
    ...
    return (
        "Task: extract likely API entrypoint declarations from provided files only. "
        "This includes HTTP routes, gRPC service RPC methods, and GraphQL query/mutation resolvers.\n"
        "Rules:\n"
        "- Return only entrypoint declarations or framework registrations -- never invocations/calls.\n"
        "- Do NOT return HTTP requests, test client calls, outbound client calls, docs/examples, curl commands, or OpenAPI examples.\n"
        "- Do NOT return an RPC client call, a GraphQL client query string, or a generated stub/client method -- only the SERVER-side handler declaration.\n"
        ...
        "- Set \"kind\" to \"http\" (default), \"grpc\", or \"graphql\" depending on what the declaration actually is.\n"
        "- For \"grpc\": set method to \"GRPC\", path to \"<ServiceName>/<MethodName>\", handler to the implementing method/function.\n"
        "- For \"graphql\": set method to \"QUERY\" or \"MUTATION\" (whichever the field is), path to the field name, handler to the resolver method/function.\n\n"
        ...
        "- gRPC (kind=\"grpc\"): a generated `*ServiceServer`/`*ServiceBase` interface method implementation "
        "(e.g. Go `func (s *server) LoginUser(ctx, req *pb.LoginUserRequest) (*pb.LoginUserResponse, error)`, "
        "a Java class implementing a generated `*Grpc.*ImplBase`, a Python class implementing a `*Servicer`).\n"
        "- GraphQL (kind=\"graphql\"): a resolver method/function bound to a Query/Mutation type "
        "(e.g. Java Spring `@MutationMapping`/`@QueryMapping`/`@SchemaMapping`, a `graphql-java` `DataFetcher`, "
        "a resolver class/method named after the schema's `Query`/`Mutation` type).\n\n"
        ...
        "- a generated gRPC/GraphQL client stub method call, or a `.graphql`/`.proto` query/mutation document sent by a caller\n\n"
        "Return JSON only.\n"
        "Use either:\n"
        '{"endpoints":[{"method":null,"path":null,"handler":null,"file":"","repo":"","service":null,'
        '"evidence":[{"file":"","symbol":null,"label":"declaration line"}],"confidence":null,"status":null,"kind":"http"}],"notes":[]}\n'
