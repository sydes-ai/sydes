...
def bridge_interface_call_edges(route_index: dict[str, Any], call_edges: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    The synthetic edge's `callee_qualified_name` is built the same way Sydes'
    own change-symbol attribution builds one for a Java method — `f"{class}.
    {method}"` — because that, not CBM's own longer form, is what a changed
    symbol's identity is actually keyed by (`ImpactInterpreter`'s
    `identity_of_symbol` trusts any qualified name containing a `.`
    verbatim). Using CBM's raw form here would build an edge that resolves to
    a different identity than the changed symbol it is meant to reach.
    """
    ...
    for edge in call_edges:
        ...
        bridged.append({
            **edge,
            ...
            "callee_qualified_name": f"{impl_class}.{callee_symbol}",
            "callee_line": None,
