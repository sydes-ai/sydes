...
def _importance_score(name: str, statement: dict) -> int:
    lowered = name.lower()
    score = 0
    if any(token in lowered for token in ("service", "repository", "repo", "upload", "storage", "client", "api")):
        score += 3
    if "await_call" in statement.get("signals", []):
        score += 2
    if "response_return" in statement.get("signals", []):
        score += 1
    if any(token in lowered for token in ("humanfilesize", "getkey", "getstorageurl")):
        score -= 3
    return score
