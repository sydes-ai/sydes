...
- Also identify tests that DIRECTLY exercise the changed behavior (construct or call the changed code, or exercise the entrypoint that reaches it) — not merely a test file that happens to be part of this diff.

Evidence rules (important):
- Every conclusion must be supported by repository evidence you actually read with your tools: an exact construction/dispatch of a message, a matching registration/decorator/configuration, a direct call or reference, a concrete route definition, an explicit handler registration, a test directly constructing/calling the changed code, or source/config wiring showing a real runtime connection.
- Do NOT mark a path "established" on "seems likely", "probably", or "framework convention suggests" reasoning alone. Vague plausibility is not evidence.
...
{"final": {
  "recovered_paths": [
    {"entrypoint": "e.g. GET /users or a queue/job name", "steps": [{"symbol": "...", "file": "...", "relationship": "e.g. constructs and dispatches"}], "evidence": [{"file": "...", "line_start": 1, "line_end": 5, "fact": "short specific statement of what these lines show"}], "status": "established" | "unresolved"}
  ],
  "recovered_tests": [
...
def recover(
    ...
    *,
    ...
    if verified.status == STATUS_UNRESOLVED and any(
        p.rejection_reason for p in verified.recovered_paths
    ) and stats.verify_retries < active_budget.max_verify_retries:
        rejection_notes = "; ".join(
            p.rejection_reason for p in verified.recovered_paths if p.rejection_reason
        )
        ...
        try:
            ...
            if retry_verified.status == "established" or not verified.recovered_paths:
                verified = retry_verified
        except RecoveryError:
            pass  # keep the original (unresolved) verified result rather than losing it
